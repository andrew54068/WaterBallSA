        ...
        for (Map<String, String> row : rows) {
            ...
            if (row.containsKey("validFrom")) {
                ...
                if (resolved instanceof LocalDateTime) {
                    builder.validFrom((LocalDateTime) resolved);
                } else if (resolved != null) {
                    builder.validFrom(resolver.convertValue(resolved.toString(), LocalDateTime.class));
                }
            }

            if (row.containsKey("validTo")) {
                Object resolved = resolver.resolveVariable(row.get("validTo"));
                if (resolved instanceof LocalDateTime) {
                    builder.validUntil((LocalDateTime) resolved);
                } else if (resolved != null) {
