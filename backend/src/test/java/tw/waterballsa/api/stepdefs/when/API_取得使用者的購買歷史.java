...
public class API_取得使用者的購買歷史 {
    ...
    private static final Set<String> QUERY_PARAMS = Set.of("page", "size", "sortBy", "direction");
    private static final Set<String> HEADER_PARAMS = Set.of();

    @When("\\(UID={string}) 取得使用者的購買歷史, call table:")
    public void invoke(String actorId, DataTable dataTable) throws Exception {
        ...
        // 2. Resolve actor ID (may be variable reference like "$userId" or constant)
        Object resolvedActorId = resolver.resolveVariable(actorId);
        String token = authenticator.getToken(resolvedActorId);
        ...
        String url = "/purchases/my-purchases";
