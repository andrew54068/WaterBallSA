...
public class Then_應該存在一個VideoProgress_With_Table {
    ...
    public void validate(DataTable dataTable) throws Exception {
        ...
        for (Map<String, String> row : rows) {
            VideoProgress actual = null;
            Object resolvedId = null;

            if (row.containsKey("id")) {
                String id = row.get("id");
                resolvedId = resolver.resolveVariable(id);
            ...
            } else if (row.containsKey("userId") && row.containsKey("lessonId")) {
                String userIdStr = row.get("userId");
                Object resolvedUserId = resolver.resolveVariable(userIdStr);
                Long userId = (resolvedUserId instanceof Number) ? ((Number) resolvedUserId).longValue()
                        : Long.parseLong(resolvedUserId.toString());

                String lessonIdStr = row.get("lessonId");
                Object resolvedLessonId = resolver.resolveVariable(lessonIdStr);
                Long lessonId = (resolvedLessonId instanceof Number) ? ((Number) resolvedLessonId).longValue()
                        : Long.parseLong(resolvedLessonId.toString());

                // Use stream filter as fallback if method not in repository interface
                actual = repository.findAll().stream()
                        .filter(vp -> vp.getUserId().equals(userId) && vp.getLessonId().equals(lessonId))
                        .findFirst()
                        .orElseThrow(() -> new AssertionError(
                                "VideoProgress with userId=" + userId + ", lessonId=" + lessonId + " should exist"));
            } else {
                throw new IllegalArgumentException("DataTable 必須包含欄位: id OR (userId & lessonId)");
            }
            ...
            if (row.containsKey("userId")) {
                ...
                if (resolved != null) {
                    ...
                    int actualInt = (v instanceof Number) ? ((Number) v).intValue()
            ...
            if (row.containsKey("completionPercentage")) {
                ...
                if (resolved != null) {
                    ...
                    BigDecimal actualNum = new BigDecimal(String.valueOf(v));
                    BigDecimal expectedNum = new BigDecimal(resolved.toString());
                    assertEquals(0, expectedNum.compareTo(actualNum),
                            "VideoProgress.completionPercentage should match");
                }
            }
            ...
            if (row.containsKey("isCompleted")) {
                ...
                if (resolved != null) {
                    if (resolved.toString() == null || resolved.toString().isEmpty()) {
                        ...
                        boolean actualBool = (v instanceof Boolean) ? ((Boolean) v).booleanValue()
