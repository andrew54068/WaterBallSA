Feature: 單元內容與試看
  ...
  Background:
    ...
    And 準備一個Lesson, with table:
      | >Lesson.id | chapterId   | title          | lessonType | isFreePreview |
      | <lessonId  | $Chapter.id | 變數與資料型別 | VIDEO      | true          |

  Rule: 使用者可以查詢單元詳情
    Example: 查詢單元詳情由應回傳完整資訊
      When 取得單元詳情, call table:
        | id         |
        | $Lesson.id |
      Then 回應, with table:
        | id         | title          | lessonType | isFreePreview |
        | $Lesson.id | 變數與資料型別 | VIDEO      | true          |
